...
class FileFeatureStorage(FileStorageMixin, FeatureStorage):
    ...
    def write(self, data_array: Union[List, np.ndarray], index: int = None) -> None:
        ...
        if not self.uri.exists():
            ...
            with self.uri.open("wb") as fp:
                np.hstack([index, data_array]).astype("<").tofile(fp)
        else:
            if index is None or index > self.end_index:
                ...
                with self.uri.open("rb+") as fp:
                    _old_data = np.fromfile(fp, dtype="<")
                    ...
                    _df["new"].fillna(_df["old"]).values.astype("<").tofile(fp)
    ...
    def start_index(self) -> Union[int, None]:
        if not self.uri.exists():
            return None
        with self.uri.open("rb") as fp:
            index = int(np.frombuffer(fp.read(4), dtype="<")[0])
        return index
    ...
    def __getitem__(self, i: Union[int, slice]) -> Union[Tuple[int, float], pd.Series]:
        ...
        with self.uri.open("rb") as fp:
            if isinstance(i, int):
                ...
                return i, struct.unpack("", fp.read(4))[0]
            elif isinstance(i, slice):
                ...
                data = np.frombuffer(fp.read(4 * count), dtype="<")

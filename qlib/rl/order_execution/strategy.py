...
from qlib.backtest.utils import get_start_end_idx, LevelInfrastructure
        ...
        if exec_vol.sum() > self.position and exec_vol.sum() > 0.0:
            if exec_vol.sum() > self.position + 1.0:
                warnings.warn(
                    ...
                    "All execution volume is scaled down linearly to ensure that their sum does not position."
                )
